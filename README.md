...
- **专注局部改写**：划选正文后光标会自动进入要求输入区；支持多行要求和持久化默认要求。发送按钮位于默认要求操作行右侧，并沿用 DSH 状态：要求为空时显示浅蓝、输入后显示深蓝。拖动输入区或功能区的上边缘时，两者会联动且输入区下边缘固定；拖拽提示仅在悬停或聚焦时显示。
- **改写先审核再应用**：模型通过 `writing_draft` 生成候选稿，预览页会直接高亮 Diff；接受和拒绝操作位于底部状态行，离开审核时默认接受。
- **状态清晰**：已复制、已生成、待审核和失败等状态使用 DSH 主题的浅色语义胶囊展示，并与默认要求操作紧凑排列在同一行。
